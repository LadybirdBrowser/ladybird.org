---
description: Launching a nonprofit, Web Platform Tests, Image format libraries, and more
date: 2024-07-31
draft: true
---
...
We continue leaning into more of the OSS ecosystem. This month we’ve integrated the following 3rd party libraries to improve our functionality and performance:
...
- GPU acceleration\! If a device supports GPU acceleration, Skia will use the Ganesh backend with Metal context on macOS and Vulkan context on Linux.  
- Better surface allocation\! macOS uses `IOSurface` for backing stores, which means we avoid copying data from Skia's render target before passing it through IPC to the browser process to display it on the screen. In the future similar optimisation will be implemented for Linux.  
- More speed\! Before switching to Skia, the rasterization process dominated performance profiles on many websites. However, now the picture is completely different. Often, recording a display list that describes what needs to be rasterized takes longer than the rasterization process itself.  
