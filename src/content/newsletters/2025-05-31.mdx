...
We've been making solid progress on WPT as usual. This month, we squeezed out 15,961 new passing tests for a total of 1,815,223.
...
Here's a simple example to illustrate how a polymorphic cache helps us:
...
Before, we had implemented them using a trampoline function. This meant that two stack frames were created. By avoiding the need for a trampoline, we've cut it down to a single stack frame.
...
Continuing from the work on descriptors last month, we now partially implement `@page` rules, which allows websites to control the page styling when they are printed. We don't actually support printing yet, but once we do, some of the groundwork for applying style will already be there.
