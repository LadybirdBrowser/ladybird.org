...
Progress on WPT continues. This month we managed to squeeze out 8,106 new passes for a total of 1,839,962.

### Google Sheets support

This month we got Google's spreadsheet application running in Ladybird. There's a lot of correctness and performance work remaining here, but it's great to see major web apps start working.
...
Ladybird's layout logic needs to take all kinds of nested boxes and inline contents into account when determining where
everything needs to go. We were still lacking support for certain nested inline contents; for example, a nested `<span>`
